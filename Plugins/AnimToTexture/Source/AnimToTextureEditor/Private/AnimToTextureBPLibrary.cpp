	...
	const FTransform RootTransform, const bool bCreateTextures, const bool bCreateUVChannel)
{
	...
	if (DataAsset->Mode == EAnimToTextureMode::Bone)
	{
		...
		NumBones = AnimToTexture_Private::GetNumBones(BaseMeshComponent->GetSkinnedAsset());

		// Get Raw Ref Bone (no virtual)
		TArray<FTransform> RefBoneTransforms;
		AnimToTexture_Private::GetRefBoneTransforms(BaseMeshComponent->GetSkinnedAsset(), RefBoneTransforms);
		...
		AnimToTexture_Private::GetBoneNames(BaseMeshComponent->GetSkinnedAsset(), BoneNames);
	...
	for (const FAnimSequenceInfo& AnimSequenceInfo : DataAsset->AnimSequences)
	{
		...
		if (AnimSequence->GetSkeleton() != BaseMeshComponent->GetSkinnedAsset()->GetSkeleton())
		{
			UE_LOG(LogTemp, Warning, TEXT("Invalid AnimSequence: %s for given SkeletalMesh: %s"), *AnimSequence->GetFName().ToString(), *BaseMeshComponent->GetSkinnedAsset()->GetFName().ToString());
			continue;
		}
		// Set AnimSequence
		else
