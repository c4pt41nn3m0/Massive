...
int32 GetVertices(const USkinnedAsset& Mesh, const int32 LODIndex, 
...
void GetStaticToSkeletalMapping(
	...
	const USkinnedAsset& SkeletalMesh, const int32 SkeletalLODIndex,
...
int32 GetNumBones(const USkinnedAsset* SkeletalMesh)
{
	check(SkeletalMesh);
...
*/
void GetRefBoneTransforms(const USkinnedAsset* SkeletalMesh, TArray<FTransform>& Transforms)
{
	check(SkeletalMesh);
	Transforms.Reset();
...
bool HasBone(const USkinnedAsset* SkeletalMesh, const FName& Bone)
{
	check(SkeletalMesh);
...
void GetBoneNames(const USkinnedAsset* SkeletalMesh, TArray<FName>& OutNames)
{
	check(SkeletalMesh);
	OutNames.Reset();
	...
	TArray<FVector3f>& OutPositions, TArray<FVector3f>& OutNormals)
{
	...
	// Get SkeletalMesh
	USkinnedAsset* SkinnedAsset = MeshComponent->GetSkinnedAsset();
	if (!SkinnedAsset)
	{
		return;
	}
	...
	TArray<FVector3f> Normals;
	const int32 NumVertices = GetVertices(*SkinnedAsset, LODIndex, Vertices, Normals);

	// TODO: Add Morph Deltas to Vertices.

	// Get Weights
	TArray<TVertexSkinWeight<MAX_TOTAL_INFLUENCES>> SkinWeights;
	GetSkinWeightsData(*SkinnedAsset, LODIndex, SkinWeights);
...
void GetSlaveBoneMap(const USkinnedAsset* MasterSkeletalMesh, const USkinnedAsset* SlaveSkeletalMesh, TArray<int32>& SlaveBoneMap)
{
...
void GetSkinWeightsData(const USkinnedAsset& SkeletalMesh, const int32 LODIndex, TArray<TVertexSkinWeight<MAX_TOTAL_INFLUENCES>>& SkinWeights)
{
	// Reset Weights
	SkinWeights.Reset();
